...
try {
    ...
    // Build the SQL query
    // Only fetch students with approved IO status
    $sql = "SELECT s.id, s.first_name, s.last_name, s.email, s.nationality, 
                   s.country, s.passport_no, s.ao_status as status, s.created_at,
                   CONCAT(s.first_name, ' ', s.last_name) as full_name,
                   s.programme_code_1
            FROM students s
            LEFT JOIN student_login sl ON s.id = sl.student_id
            WHERE s.io_status = 'approved'";
    ...
    // Add AO status filter if provided
    if (!empty($status)) {
        $sql .= " AND s.ao_status = ?";
        $params[] = $status;
        $types .= 's';
    ...
    // Count total records
    $count_sql = "SELECT COUNT(*) as total FROM students s WHERE s.io_status = 'approved'";
    $count_params = [];
    $count_types = '';
